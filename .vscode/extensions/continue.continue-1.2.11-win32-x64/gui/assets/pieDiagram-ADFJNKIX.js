...
`,"getStyles"),be=xe,De=u(e=>{const a=[...e.values()].reduce((s,o)=>s+o,0),g=[...e.entries()].map(([s,o])=>({label:s,value:o})).filter(s=>s.value/a*100>=1).sort((s,o)=>o.value-s.value);return de().value(s=>s.value)(g)},"createPieArcs"),Ae=u((e,a,g,w)=>{_.debug(`rendering pie chart
`+e);const s=w.db,o=Q(),l=X(s.getConfig(),o.pie),t=40,n=18,c=4,p=450,S=p,m=ee(a),d=m.append("g");d.attr("transform","translate("+S/2+","+p/2+")");const{themeVariables:i}=o;let[y]=te(i.pieOuterStrokeWidth);y??(y=2);const x=l.textPosition,h=Math.min(S,p)/2-t,A=L().innerRadius(0).outerRadius(h),T=L().innerRadius(h*x).outerRadius(h*x);d.append("circle").attr("cx",0).attr("cy",0).attr("r",h+y/2).attr("class","pieOuterCircle");const f=s.getSections(),b=De(f),D=[i.pie1,i.pie2,i.pie3,i.pie4,i.pie5,i.pie6,i.pie7,i.pie8,i.pie9,i.pie10,i.pie11,i.pie12];let $=0;f.forEach(r=>{$+=r});const z=b.filter(r=>(r.data.value/$*100).toFixed(0)!=="0"),E=le(D);d.selectAll("mySlices").data(z).enter().append("path").attr("d",A).attr("fill",r=>E(r.data.label)).attr("class","pieCircle"),d.selectAll("mySlices").data(z).enter().append("text").text(r=>(r.data.value/$*100).toFixed(0)+"%").attr("transform",r=>"translate("+T.centroid(r)+")").style("text-anchor","middle").attr("class","slice"),d.append("text").text(s.getDiagramTitle()).attr("x",0).attr("y",-400/2).attr("class","pieTitleText");const G=[...f.entries()].map(([r,I])=>({label:r,value:I})),k=d.selectAll(".legend").data(G).enter().append("g").attr("class","legend").attr("transform",(r,I)=>{const W=n+c,B=W*G.length/2,V=12*n,U=I*W-B;return"translate("+V+","+U+")"});k.append("rect").attr("width",n).attr("height",n).style("fill",r=>E(r.label)).style("stroke",r=>E(r.label)),k.append("text").attr("x",n+c).attr("y",n-c).text(r=>s.getShowData()?`${r.label} [${r.value}]`:r.label);const P=Math.max(...k.selectAll("text").nodes().map(r=>(r==null?void 0:r.getBoundingClientRect().width)??0)),R=S+t+n+c+P;m.attr("viewBox",`0 0 ${R} ${p}`),ae(m,p,R,l.useMaxWidth)},"draw"),Te={draw:Ae},Ge={parser:Se,db:O,renderer:Te,styles:be};export{Ge as diagram};
